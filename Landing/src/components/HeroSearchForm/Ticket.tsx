...
export interface TicketProps {
  ...
  onChange?: (data: TicketProps["defaultValue"]) => void;
          ...
          >
            ...
            <div className="flex-grow">
              <span className="block xl:text-lg font-semibold">
                {totalGuests || ""} Ingressos
              </span>
              <span className="block mt-1 text-sm text-neutral-400 leading-none font-light">
                {totalGuests ? "Ingressos" : "Adicionar Ingressos"}
              </span>
              {!!totalGuests && open && (
                <ClearDataButton
                  onClick={() => {
          ...
          >
            <Popover.Panel className="absolute right-0 z-10 w-full sm:min-w-[340px] max-w-sm bg-white dark:bg-neutral-800 top-full mt-3 py-5 sm:py-6 px-4 sm:px-8 rounded-3xl shadow-xl">
              ...
              <NcInputNumber
                ...
                max={4}
                label="Meia entrada"
                desc="Ingressos de meia entrada"
              />
            </Popover.Panel>
          </Transition>
        </>
